    ...
    If ($cluster -or $switch_name -eq 'Management') {
        ...
        Else {
            ...
            If ($switch_name -eq 'Management') {
                ...
                $vswitch = New-VMSwitch -Name $switch_name -NetAdapterName $p_nic_array[0].Name -EnableEmbeddedTeaming $true -EnableIov $true -MinimumBandwidthMode Weight -AllowManagementOS $true
            ...
            ElseIf ($cluster) {
                ...
                $vswitch = New-VMSwitch -Name $switch_name -NetAdapterName $p_nic_array[0].Name -EnableEmbeddedTeaming $true -EnableIov $true -MinimumBandwidthMode Weight
                For ($i = 1; $i -lt $p_nic_array.Count; $i++) {
                    Write-Host ($hostname_vm + ',' + $switch_name + ' - expanding switch with: ' + $p_nic_array[$i].Name)
                    Add-VMSwitchTeamMember -SwitchName $switch_name -NetAdapterName $p_nic_array[$i].Name
                }
