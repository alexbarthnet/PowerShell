...
Try {
	...
	$map_network | Where-Object { $_.VmPath -and $_.VhdPath } | ForEach-Object {
		If ($cluster) {
			Write-Host ("$Hostname - Host is clustered, configuring Hyper-V paths...")
		...
		$map_network | Where-Object { $_.vNIC } | ForEach-Object {
			...
			If ($vswitch) {
				...
				If ($nic_qos) {
					...
					Else {
						Write-Host ("$Hostname,$vswitch_name,$vnic_name - QoS disabled, fixing...")
				...
				Write-Host ("$Hostname,$vswitch_name,$vnic_name - Adding VNIC as VM migration network")
