<#
.SYNOPSIS
Updates the physical networking on a Hyper-V host.

.DESCRIPTION
Updates the physical networking on a Hyper-V host with information from a set of host-specific configuration files. A parent script pushes this script and the configuration files to each Hyper-V host then starts the script using PowerShell Remoting.
...
Write-Host ("$host_name - Disabling DHCP on non-USB NICs...")
$nics_not_usb = $null
...
ForEach ($nic_not_usb in $nics_not_usb) {
	...
	If ($nic_dhcp_v4) {
		Write-Host ("$host_name, $nic_ifalias - Disabling DHCP v4 and restarting NIC")
		$nic_dhcp_v4 | Set-NetIPInterface -Dhcp Disabled
		$nic_not_usb | Restart-NetAdapter
	}
	Else {
		Write-Host ("$host_name, $nic_ifalias - DHCPv4 disabled")
	}
}
...
$map_network | ForEach-Object {
	...
	If ($nic_exists) {
		...
		If ($nic_bound) {
			...
			Write-Host ("$host_name, $nic_name, $nic_addr - IPv4 bound, checking IP addresses and gateway...")
			...
			If ($nic_addr_on_nic) {
				Write-Host ("$host_name, $nic_name, $nic_addr - ...wrong IP address found on current NIC, removing '$($nic_addr_on_nic.IPv4Address)' from '$($nic_addr_on_nic.InterfaceAlias)'")
			...
			If ($nic_addr_on_sys) {
				Write-Host ("$host_name, $nic_name, $nic_addr - ...current IP address found on other NIC, removing '$($nic_addr_on_sys.IPv4Address)' from '$($nic_addr_on_sys.InterfaceAlias)'")
			...
			If ($nic_correct_ip) {
				# IP address found
				Write-Host ("$host_name, $nic_name, $nic_addr - ...current IP address found on correct physical or virtual NIC")
			}
			Else {
			...
			If ($nic_gway -eq '0') {
				...
				If ($nic_wrong_gw) {
					Write-Host ("$host_name, $nic_name, $nic_addr - ...current NIC has default route and should not, removing route to '$($nic_wrong_gw.DestinationPrefix)' from '$($nic_wrong_gw.InterfaceAlias)'")
			...
			Else {
				...
				$nic_route | ForEach-Object {
					Write-Host ("$host_name, $nic_name, $nic_addr - ...gateway found on other NIC, removing route to '$($_.DestinationPrefix)' from '$($_.InterfaceAlias)'")
				...
				If ($nic_correct_gw) {
					Write-Host ("$host_name, $nic_name, $nic_addr - ...default gateway found on correct physical or virtual NIC")
				}
				Else {
				...
				Write-Host ("$host_name, $nic_name, $nic_addr - ...setting DNS servers: $(Get-Content($dns_servers) -join ',')")
		...
		If ($nic_qos) {
			...
			Else {
				Write-Host ("$host_name, $nic_name, $nic_addr - QoS not enabled, disabling NIC to enable QoS...")
				$nic_exists | Disable-NetAdapter -Confirm:$false
				Start-Sleep -Seconds 5
				Write-Host ("$host_name, $nic_name, $nic_addr - ...NIC disabled, enabling QoS...")
				$nic_qos | Enable-NetAdapterQos
				Start-Sleep -Seconds 15
				Write-Host ("$host_name, $nic_name, $nic_addr - ...QoS enabled, enabling NIC...")
