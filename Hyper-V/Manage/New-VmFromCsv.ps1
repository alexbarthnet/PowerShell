...
$vm_list | ForEach-Object {
    ...
    $vhd_data_gb = $_.DataVhdGB
    $vhd_excl_ct = $_.ExclVhdCount
    $vhd_excl_gb = $_.ExclVhdGB
    ...
    $vm_osd_domain = $_.OsdDomain
    $vm_osd_path = $_.OsdPath

    # define optional objects from CSV - SCCM collections
    $vm_col_deploy = $_.ColDeploy
    $vm_col_window = $_.ColWindow
    ...
    Else {
        ...
        # check if VM was found, make VM if not found
        If ($vm_on_hv) {
            Write-Host ("$env_comp_name,$vm_host,$vm_name - ....VM already exists on host")
        }
        Else {
    ...
    If ($vm_host -match 'cloud') {
        Write-Host ("$env_comp_name,$vm_host,$vm_name - VM is in the cloud, skipping DHCP step...")
    }
    ElseIf ($vm_dhcp_server -and $vm_dhcp_scope -and $vm_ip_address -and $vm_hw_address) {
        ...
        If ($vm_dhcp) {
            Write-Host ("$env_comp_name,$vm_host,$vm_name - removing existing DHCP reservation")
            $vm_dhcp | Remove-DhcpServerv4Reservation -ComputerName $vm_dhcp_server
        }

        # create DHCP reservation
        $vm_macaddr = ($vm_hw_address -split '(\w{2})' | Where-Object { $_ -ne '' }) -join '-'
        $vm_dnstail = ($vm_osd_oupath -split ',' | Where-Object { $_ -match 'DC=' }) -join '.' -replace 'DC=', ''
        $vm_dnsname = ($vm_name + '.' + $vm_dnstail).ToLower()
        Write-Host ("$env_comp_name,$vm_host,$vm_name - creating DHCP reservation with MAC: " + $vm_macaddr)
        Add-DhcpServerv4Reservation -ComputerName $vm_dhcp_server -Name $vm_dnsname -ScopeId $vm_dhcp_scope -IpAddress $vm_ip_address -ClientId $vm_macaddr
    }
    Else {
    ...
    If ($vm_prov) {
        ...
        switch ($vm_osd_method) {
            'iso' {
                ...
                If ($vm_osd_path) {
                    ...
                    If ($vm_host_found_iso) {
                        # Write-Host ("$env_comp_name,$vm_host,$vm_name - ...retrieving DVD drive")
                        # $vm_dvd_drive = Get-VMDvdDrive -VM $vm
            ...
            'wds' {
                # pre-stage VM in WDS
                $vm_biosguid = $vm_data_object.BIOSGUID
                If ($vm_osd_server -and $vm_biosguid -and $vm_osd_path) {
                ...
                Else {
                    Write-Host ("$env_comp_name,$vm_host,$vm_name - WDS server not provided, skipping WDS provisioning...")
                }
            }
            'sccm' {
                If ($vm_switch -eq 'Remove') {
                ...
                Else {
                    ...
                    Invoke-Command -ComputerName $vm_osd_server -ScriptBlock {
                        ...
                        Else {
                            ...
                            New-CMDeviceVariable -DeviceId $dev_resid -VariableName 'OSDDOMAIN' -VariableValue $dev_domain | Out-Null
                            Write-Host ("$env_name,$sccm_srv,$dev_name - ...setting OSD OU name to: " + $dev_oupath)
                            New-CMDeviceVariable -DeviceId $dev_resid -VariableName 'OSDDOMAINOUNAME' -VariableValue $dev_oupath | Out-Null
                        }
                    }
                }
            }
