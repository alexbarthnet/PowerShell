...
Process {
	...
	ForEach ($Name in $VMObjects.Keys) {
		...
		If ($null -ne $VMSnapshot) {
			...
			# restore VM
			Try {
				Write-Host ("$Hostname,$ComputerName,$Name - restoring checkpoint for VM...")
				Restore-VMSnapshot @RestoreVMSnapshot
			}
			Catch {
				Write-Host ("$Hostname,$ComputerName,$Name - ERROR: restoring checkpoint VM")
				Throw $_
			}

			# report
			Write-Host ("$Hostname,$ComputerName,$Name - ...restored checkpoint for VM")
		}
	}
