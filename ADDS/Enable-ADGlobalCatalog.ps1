...
<#
.SYNOPSIS
Enable the Global Catalog role for the local system on a remote domain controller.
...
Process {
	...
	If ($ADObject.options -eq '1') {
		Write-Verbose -Verbose -Message "found Global Catalog role already enabled for local domain controller on remote domain controller: $Server"
		Return
	}
	...
	# declare complete
	Write-Verbose -Verbose -Message "enabled Global Catalog role for local domain controller on remote domain controller: $Server"
}
